...
ui_filename = "andor_control.ui" # filename here
...
class AcquisitionWidget(gui.QWidget, Ui_Widget):
    ...
    def __init__(self, cam, acquiring=False):
        ...
        # First name tabs
        self.tabDevices.setTabText(0, "Cameras")
        self.tabDevices.setTabText(1, "Spectrographs")

        # SetupUi crashes on QComboBoxes, so add them manually here
        self.cam = cam
        #self.nameLabel.setText(self.cam.name)
        self.addComboBoxToFormLayout(self.formLayoutCams, 0, 
            "editCameraComboBox", ["Newton", "iDus"], enabled=True)
        self.addComboBoxToFormLayout(self.formLayoutCams, 1, 
            "activeCameraComboBox", ["Newton", "iDus"], enabled=True)
        self.addComboBoxToFormLayout(self.formLayoutCams, 2, 
            "acquisitionModeComboBox", ["Scan until abort"], enabled=False)
        self.addComboBoxToFormLayout(self.formLayoutCams, 3, 
            "readModeComboBox", ["Full vertical binning"], enabled=False)
        self.addComboBoxToFormLayout(self.formLayoutCams, 9, 
            "triggerSourceComboBox", ["Internal", "External"], enabled=True)
        self.addComboBoxToFormLayout(self.formLayoutCams, 10, 
            "keepCleanModeComboBox", ["Wait", "Trigger abort"], enabled=False)
        # ONLY AVAILABLE ON NEWTON IN FVB EXTERNAL TRIGGER MODE
        if self.cam.name == "newton":
            self.keepCleanModeComboBox.addItem("Disable")

        self.addComboBoxToFormLayout(self.formLayoutSpecs, 0, 
            "editSpectrographComboBox", ["Shamrock"], enabled=True)
        self.addComboBoxToFormLayout(self.formLayoutSpecs, 1, 
            "activeSpectrographComboBox", ["Shamrock"], enabled=True)
        self.addComboBoxToFormLayout(self.formLayoutSpecs, 2, 
            "activeGratingComboBox", ["1", "2"], enabled=True)
    ...
    def addComboBoxToFormLayout(self, layout, row, name, menu_items, enabled=True):
        ...
        # print(item)
        # if item is not None:
        #     print("row: " + str(row) + "item: " + str(item.widget().objectName()))

        layout.setWidget(row, 1, getattr(self, name))
