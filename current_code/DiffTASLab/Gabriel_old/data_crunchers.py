...
def calcPumpProbeRatioSimple(po, pp):
...
def calcPumpProbeRatioFFT(po, pp, ftbuf):
    # calculate the pump-probe signal in terms of ratio between pump-probe
    # and probe-only signal
...
def calcAverageStddevRms(data, start, boxcar):
    ...
    average = np.mean(data[:, start:start+boxcar-1], axis=1)
    stddev  = np.std(data[:, start:start+boxcar-1], axis=1, ddof=1)
    rms     = stdev / average

    return average, stddev, rms


def calcAverageStddevRmsStruct(out, data, start, boxcar):
    # calculate average, standard deviation, and rms error
    # and return everything together with the data matrix in a structure
    
    out.data    = data
    out.average = np.mean(data[:, start:start+boxcar-1], axis=1)
    out.stddev  = np.std(data[:,  start:start+boxcar-1], axis=1, ddof=1)
    out.rms     = stdev / average
