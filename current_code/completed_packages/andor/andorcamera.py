...
import threading
import numpy as np
from .andordll import cam_lib, spec_lib
...
class AndorCamera(object):
    ...
    def __init__(self, serial, spec, out=print):
        ...
        """Initialize parameter dictionary for acqusition settings;
        this will be addressed by setting functions like prep_acquisition.
        The actual values (retrieved by get* functions) will be updated here, 
        so that the acqusition functions (expose etc.) do not mess around
        the values, just start/stop measurements.
        """
    ...
    def initialize(self):
        ...
        # get default set temperature
        self.has_edit_focus = True
        self.parms["set_temperature"]   = float(cam_info[self.name]["temp"])
        self.parms["temp_limits"]       = self.get_temp_range()
        self.has_edit_focus = False
    ...
    def cooldown(self, temp=None):
        ...
        if not self.has_edit_focus:
            # wrong camera, do nothing
            return
    ...
    # def set_parameters(self, acq_mode, read_mode, exp_time=0,
    #                      accum_cycle_time=None, n_accums=None,
    #                      kin_cycle_time=None, n_kinetics=None,
    #                      trigger="external", keep_clean_mode="wait",
    #                      slitwidth=None, wavelen=None):
    def set_acq_parameters(self, parms):
        ...
        # check if spectrograph was initialized, if not, do it now...
        # strange, because this should not happen, but seems to be ...
        # if not self.spec.is_initialized():
        #     print("re-initializing shamrock\n")
        ...
        #print("Setting flipper mirror: " + str(self.name) + str(self.spec.name)+ str(cam_info[self.name]["port"]) + "\n")
        ...
        cam_lib.SetAcquisitionMode(self.acq_modes[parms["acq_mode"]])
        self.out("Setting read mode:", end=" ")
        cam_lib.SetReadMode(self.read_modes[parms["read_mode"]])

        # Set timing-related parameters
        self.out("Setting exposure time:", end=" ")
        cam_lib.SetExposureTime(float(parms["exp_time"]) / 1000)
        if parms["accum_cycle_time"] is not None:
            self.out("Setting accumulation cycle time:", end=" ")
            cam_lib.SetAccumulationCycleTime(float(parms["accum_cycle_time"]) / 1000)
        if parms["n_accums"] is not None:
            self.out("Setting number of accumulations:", end=" ")
            cam_lib.SetNumberAccumulations(parms["n_accums"])
        if parms["kin_cycle_time"] is not None:
            self.out("Setting kinetic cycle time:", end=" ")
            cam_lib.SetKineticCycleTime(float(parms["kin_cycle_time"]) / 1000)
        if parms["n_kinetics"] is not None:
            self.out("Setting number of kinetics:", end=" ")
            cam_lib.SetNumberKinetics(parms["n_kinetics"])
        self.out("Setting trigger mode:", end=" ")
        cam_lib.SetTriggerMode(self.trigger_modes[parms["trigger"]])
        get_set_keep_cleans = (self.name == "Newton01" and parms["read_mode"] == "fullbin"
            and parms["trigger"] == "external")
        if parms["keep_clean_mode"] == "wait":
            if get_set_keep_cleans:
                cam_lib.EnableKeepCleans(1)
            cam_lib.SetFastExtTrigger(0)
        elif parms["keep_clean_mode"]== "trigger_abort":
            if get_set_keep_cleans:
                cam_lib.EnableKeepCleans(1)
            cam_lib.SetFastExtTrigger(1)
        elif parms["keep_clean_mode"]=="disable":
            if get_set_keep_cleans:
        ...
        parms["exp_time"]           = actual_times[0] * 1000
        if parms["accum_cycle_time"] is not None:
            parms["accum_cycle_time"]   = actual_times[1] * 1000
        if parms["kin_cycle_time"] is not None:
            parms["kin_cycle_time"]     = actual_times[2] * 1000
        parms["readout_time"]       = cam_lib.GetReadOutTime(float) * 1000

        if get_set_keep_cleans:
            parms["keep_clean_time"] = cam_lib.GetKeepCleanTime(float) * 1000
        else:
            parms["keep_clean_time"] = -1.
        
        #print(str(self.spec.name) + " " + str(spec_lib.ShamrockGetSlit(self.spec)) + "\n")
            
        # Set up spectrometer
        if parms["slitwidth"] is not None:
            self.out("Setting slit width:", end=" ")
            self.spec.ShamrockSetSlit(float(parms["slitwidth"]))

        if parms["grating_number"] is not None:
            self.spec.ShamrockSetGrating(int(parms["grating_number"]))
        if parms["center_wavelength"] is not None:
            self.spec.ShamrockSetWavelength(float(parms["center_wavelength"]))

        parms["grating_number"]     = self.spec.ShamrockGetGrating(int)
        parms["slitwidth"]          = self.spec.ShamrockGetSlit(float)
        parms["center_wavelength"]  = self.spec.ShamrockGetWavelength(float)
        wl_array                    = self.get_wavelen_array()
        parms["start_wavelength"]   = wl_array[0]
        parms["end_wavelength"]     = wl_array[-1]
        
        # write updated valid parms to cam object
        self.parms = parms
    ...
    def prep_acquisition(self, acq_mode, read_mode, exp_time=0,
        ...
        if slitwidth is not None:
            self.out("Setting slit width:", end=" ")
            self.spec.ShamrockSetSlit(float(slitwidth))
    ...
    def single_scan(self, read_mode="fullbin", dark=False, **kwargs):
        ...
        # set acquisition mode to "single" for this one shot
        cam_lib.SetAcquisitionMode(self.acq_modes["single"])
        actual_time = self.parms["exp_time"] + self.parms["readout_time"]
        #print("Exptime+readout: " + str(actual_time) + "\n")
        
        # Return data from single exposure
        data = self.expose(read_mode, get_data_dt=actual_time, dark=dark)

        # reset acquisition mode
        cam_lib.SetAcquisitionMode(self.acq_modes[self.parms["acq_mode"]])
    ...
    def scan_until_abort(self, alloc, read_mode="fullbin", dark=False,
        **kwargs):
        ...
        kin_time = self.parms["kin_cycle_time"]
        ...
        # 10 Hz
        #interval = min(int(kin_time*100), 100)
        interval = min(int(kin_time/10), 100)
        self.n_saved = 0
        self.timer = self.TimerClass(interval, self.cont_get_data,
...
class QAndorCamera(QAndorObject, AndorCamera):
    ...
    parameter_setting_done = QtCore.pyqtSignal()
    ...
    def set_acq_parameters(self, parms, is_first_time):
        """pass parms to AndorCamera's version of set_acq_parameters,
        ...
        if is_first_time == False:
            AndorCamera.set_acq_parameters(self, parms)
            self.parameter_setting_done.emit()
