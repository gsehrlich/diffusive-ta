...
def start_the_rest():
    ...
    reload(rms)
    reload(ratio)
    start_w = acq_control.SimpleControllerWidget(newton)
    start_w.show()

    rms_w = PlotterWidget(rms.RmsPlotter, "RMS/mean counts",
        newton.x, start_w.new_probe_only)
    rms_w.show()
    ...
    ratio_w = PlotterWidget(ratio.RatioPlotter, "Probe-only/pump-probe counts",
        newton.x, start_w.new_probe_only, start_w.new_pump_probe)
    ratio_w.show()
    ...
    pump_probe_w = PlotterWidget(AvgPlotter, "Pump-probe",
        newton.x, start_w.new_pump_probe)
    pump_probe_w.show()
